...
/*    @Test
    public void testParse2() {
        ...
        nodes = nodeParser.parse(path);
        final SystemUnderConsiderationGraph suc = relationParser.parse(path, nodes);
        assertEquals("Adapters", suc.getName());
        ...
        assertTrue(containsType(source, NodeType.CLASS));
        assertTrue(containsType(target, NodeType.CLASS));
        ...
        assertNotNull(findAttributeByName(target, "Double"));
        assertEquals(Visibility.PUBLIC, findAttributeByName(source, "width"));
       ...
       assertTrue(target.isAbstract());
        ...
        assertNotNull(relation);
        assertEquals(1, relation.getRelationProperties().size());
        assertTrue(relationTypeExists(relation,RelationType.RelationType.IMPLEMENTS));
